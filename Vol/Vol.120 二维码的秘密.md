...
![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751524.jpeg)

加人、购物、填表、付钱，你可能扫过无数次二维码，但却从来没有认真看过它。

![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751450.jpeg)

这个二维码结构看起来很简单，左下、左上和右上有三个方块用来定位。剩下的位置就是一堆黑色和白色小方块组成一个 29×29 的大方块，白色方块表示 0，黑色方块表示 1。由这些 0 和 1 构成的二进制字符串就可以转换成各种数字、字母和符号了。

![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751443.jpeg)

但你仔细看，就会发现没这么简单。

![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751429.jpeg)

条码系统诞生之初，就是希望创造一套简单清晰可被印刷能被机器识别的图形编码，让机器轻松获取商品编号。

这样你在超市买东西的时候，收银员就不需要手动记录，只需要扫一下就能知道多少钱了。

![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751529.jpeg)

这就是二维码的前身——一维码，今天我们都叫条形码。以 UPC（Universal Product Code）码为例，本质上就是 30 条粗细不一的黑线。

而黑色竖线和白色间隔的宽度，就是隐藏在条形码里的二进制信息。

![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751418.jpeg)
...
![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751573.jpeg)
...
![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751155.jpeg)
...
![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751186.jpeg)

最后，编号第 12 位数是根据前 11 位数计算出来的校验码，以防止篡改。

![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751676.jpeg)

这三重设计保证了条形码可以适应各种复杂的现实情况，非常可靠。

![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751410.jpeg)

但条形码上能承载的信息还是太少了，所以，以矩阵形式承载更多信息的二维码出现了。在这些奇奇怪怪的二维码中， QR 码脱颖而出，出现在了你的微信支付宝和火车票上。

![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751256.jpeg)

QR 的全称是 Quick Response，快速响应矩阵。最有代表性的就是这三个回字型方块，这样的排布方案可以让你无论从哪个方向扫描二维码都会自动校正为正确方向，只要右下角没方块就是对的，很简单。

![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751320.jpeg)
...
![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751384.jpeg)
...
![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751888.jpeg)
...
![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191752186.jpeg)
...
![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191752957.jpeg)
...
![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751530.jpeg)

我们的目标是，任选 2 格修改成其他数字，算法都可以自动发现错的是哪一个，并且还原成修改前的数据。

![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751178.jpeg)
...
![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191752462.jpeg)
...
![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191752741.jpeg)

如果 m+p-100 不等于 0，我们永远不可能知道是 m 错了还是 p 错了，但是如果我们可以把 m 和 p 组合成一个数字比如说 1234XXXX 呢？听起来不错，但这样减法就不行了，这 4 位数不管是多少都不可能得到 0，那怎么办？

![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751955.jpeg)
...
![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751140.jpeg)

这时，我们就需要改变计算规则了。隆重介绍——伽罗瓦域(Galois Field，GF)。

伽罗瓦域厉害的地方在于，这是一个封闭的世界，里面的有限个数字不管怎么算都不会得到它们之外的结果。

![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751951.jpeg)
...
![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751887.jpeg)

而乘除法的规则复杂一些，结果大于 7，比如 6*7 就需要对 110 和 111 做模二乘法，再用模二除法除以提前预设的数值 1011，得到余数 100，就是 4。

![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751757.jpeg)
...
![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751834.jpeg)

答案是：多项式。

在多项式中，我们可以把每个格子里的数字作为 x 的系数，把格子的位数作为 x 的次方数。比如 1、2、3、4 就是 1*x^3+ 2*x^2+ 3*x^1+ 4*x^0，那么 12340000 的多项式 m(x) 就是这样

![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751120.jpeg)

既然输入数据变成了多项式，我们要除以的固定值 g 也应该是一个有 x 的多项式。我们要得到的 4 位纠错码， g(x) 就有 4 个因子，分别是x减2的零次方，x减2的一次方，x减2的二次方，x减2的三次方，

![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191752609.jpeg)
...
![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751587.jpeg)
...
![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191752356.jpeg)

好，我们的准备工作已经全部完成了。现在，你可以随便修改这 8 个格子里的 2 个数，机器都可以完成自动修正。

![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751246.jpeg)
...
![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751068.jpeg)
...
![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751593.jpeg)
...
![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751262.jpeg)

现在，我们有四个方程组，四个未知数，机器就能算出来

![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751156.jpeg)

现在，我们可以把收到数据的第 5 和第 7 位的数字 2 和 6，分别和 y1 和 y2 做异或运算，得到 3 和 1，填回去，我们就得到正确的输入数据了。

需要说明的是，这是一个简化后的计算过程，实际情况还要更复杂。

![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751114.jpeg)

这就是二维码的秘密，把信息编码为二进制，按顺序填上原始数据，再填上计算后的纠错码，和其他信息，再和掩码做一次异或运算，最终成为你看到的样子，一个可靠的码。 

![img](https://cdn.jsdelivr.net/gh/just-prog/static/img/202109191751414.jpeg)
